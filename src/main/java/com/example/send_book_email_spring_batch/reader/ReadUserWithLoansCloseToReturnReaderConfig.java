...
import com.example.send_book_email_spring_batch.entities.Book;
import com.example.send_book_email_spring_batch.entities.User;
...
public class ReadUserWithLoansCloseToReturnReaderConfig {
    ...
    int numDaysNotifyReturn = GenerateBookReturnDate.numDaysToReturnBook - 1;
    ...
    public ItemReader<UserBookLoan> readUserWithLoansCloseToReturnReader(@Qualifier("appDS") DataSource dataSource) {
        ...
        return new JdbcCursorItemReaderBuilder<UserBookLoan>()
                ...
                .sql("SELECT " +
                     ...
                     "WHERE DATE_ADD(loan_date, INTERVAL " + numDaysNotifyReturn + " DAY) = DATE(NOW());")
                .rowMapper(rowMapper())
                .build();
    }
    ...
    private RowMapper<UserBookLoan> rowMapper() {
        return new RowMapper<UserBookLoan>() {
            ...
            public UserBookLoan mapRow(ResultSet rs, int rowNum) throws SQLException {
                User user = new User(rs.getLong("user_id"), rs.getString("user_name"), rs.getString("user_email"));

                Book book = new Book();
                book.setId(rs.getLong("book_id"));
                book.setName(rs.getString("book_name"));

                UserBookLoan userBookLoan = new UserBookLoan(user, book, rs.getDate("loan_date").toLocalDate());
